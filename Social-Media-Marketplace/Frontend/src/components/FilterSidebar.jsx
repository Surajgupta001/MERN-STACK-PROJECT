...
function FilterSidebar({ showFilterPhone, setShowFilterPhone, filters, setFilters }) {
    ...
    return (
        <div className={`${showFilterPhone ? 'max-sm:fixed' : 'max-sm:hidden'} bg-white border border-gray-200 rounded-lg shadow max-sm:inset-0 z-100 max-sm:h-screen max-sm:overflow-scroll-sm h-fit sticky top-24 md:min-w-[300px]`}>
            <div className='p-4 border-b border-b-gray-200'>
                <div className='flex items-center justify-between'>
                    ...
                    <div className='flex items-center gap-2'>
                        <X onClick={onClearFilters} className='p-1 text-gray-500 transition-colors rounded cursor-pointer size-6 hover:text-gray-700 hover:bg-gray-100' />
                        <button onClick={() => setShowFilterPhone(false)} className='px-3 py-1 text-sm text-gray-700 border rounded sm:hidden'>Apply</button>
                    </div>
                </div>
            </div>
            ...
            <div className='p-4 space-y-6 sm:max-h-[calc(100vh-200px)] overflow-y-scroll no-scrollbar'>
                {/* Search Box */}
                <div className='flex items-center justify-between'>
                    <input onChange={onChangeSearch} type="text" placeholder='Search by username, platform, niche, etc.' className='w-full px-3 py-2 text-sm border border-gray-300 rounded-md outline-indigo-500' />
                </div>
                {/* Platform filter */}
                <div>
                ...
                <div>
                    ...
                    {expandedSections.followers && (
                        <select onChange={(e) => onFilterChange({ ...filters, minFollowers: parseInt(e.target.value) || 0 })} value={filters.minFollowers?.toString() || "0"} className='w-full px-3 py-2 text-gray-700 border border-gray-300 rounded-lg outline-indigo-500'>
                ...
                <div>
                    ...
                    {expandedSections.niche && (
                        <select onChange={(e) => onFilterChange({ ...filters, niche: e.target.value || null })} value={filters.niche || ''} className='w-full px-3 py-2 text-gray-700 border border-gray-300 rounded-lg outline-indigo-500'>
